...
# -*- coding: utf-8 -*-
"""
app_pmo.py — PMO Unificada · Nine Fitness Brescia 19
streamlit run app_pmo.py
"""
...
# ─── CONFIG ───────────────────────────────────────────────────────
st.set_page_config(
    page_title="PMO · Nine Fitness Brescia 19",
    page_icon="📋",
    layout="wide",
...
# Detectar modo público vía URL param (?view=public)
...
# ─── TABS ─────────────────────────────────────────────────────────
if modo_publico:
    tab_names = ["📊 Dashboard", "📅 Actas Semanales"]
    tab1, tab3 = st.tabs(tab_names)
    tab2 = None
else:
    tab_names = ["📊 Dashboard", "✍️ Registrar", "📅 Consolidación Semanal", "🔗 Vista Pública"]
    tab1, tab2, tab2b, tab3 = st.tabs(tab_names)
...
with tab1:
    ...
    with fc5:
        ...
        refrescar = st.button("🔄 Actualizar", use_container_width=True, key="ref1")
...
# ══════════════════════════════════════════════════════════════════
#  TAB 2 — REGISTRAR (solo modo edición)
# ══════════════════════════════════════════════════════════════════
    ...
    with tab2:
            ...
            with st.form("form_nueva", clear_on_submit=True):
                ...
                texto_raw  = st.text_area(
                    ...
                    placeholder=(
                        ...
                        "- Mohamed (Munir): replanteo tabiquería. Salida emergencia 2.5m...\n"
                        "- Mañana 08:30 reunión contratas. 09:30 visita Pedro Hersan."
                    )
                )
        ...
        with col_prev:
            ...
            pv_txt = st.text_area("Texto a analizar", height=200, key="pv",
                                   placeholder="Pega aquí para ver cómo lo parse el sistema...")
...
# ══════════════════════════════════════════════════════════════════
#  TAB 2b — CONSOLIDACIÓN SEMANAL (solo modo edición)
# ══════════════════════════════════════════════════════════════════
if not modo_publico:
    with tab2b:
        ...
        if gen_btn:
            ...
            if borrador:
                ...
                st.success(f"Borrador S{sem_sel} generado con {len(borrador['actas_diarias_ids'].split(','))} actas.")
            else:
        ...
        # Listado de actas semanales guardadas
        st.divider()
...
# ══════════════════════════════════════════════════════════════════
#  TAB 3 — VISTA PÚBLICA / INFO COMPARTIR
# ══════════════════════════════════════════════════════════════════
with tab3 if not modo_publico else tab3:
    if not modo_publico:
        st.markdown("### 🔗 Compartir con el equipo")

        cloud_url = "https://kntwcg5w7wfjqnqqvh8izv.streamlit.app"
        pub_url   = f"{cloud_url}/?view=public"
        ...
        <div style="background:#fff;border:1px solid #e2e6f0;border-radius:10px;padding:20px 24px;margin-bottom:16px;">
          ...
          <div style="margin-bottom:10px;">
            ...
            <code style="background:#f0f2f8;padding:6px 12px;border-radius:4px;font-size:12px;">{cloud_url}</code>
          </div>
          <div>
    ...
    # Vista solo lectura de actas semanales
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("**URLs rápidas:**")
    st.code("localhost:8503", language=None)
    st.code("?view=public  ← modo público", language=None)
    st.divider()
    st.markdown("**Configurar Supabase:**  \nEdita `.streamlit/secrets.toml`")
...
CREATE TABLE actas_diarias (
  ...
  definiciones_tecnicas JSONB,
  agenda_proxima JSONB,
  incidencias JSONB,
  prioridad TEXT DEFAULT 'normal',
  estado TEXT DEFAULT 'borrador',
  texto_original TEXT,
  resumen TEXT,
  creado_por TEXT DEFAULT 'Darío A. López',
  created_at TIMESTAMPTZ DEFAULT NOW()
);
CREATE INDEX ON actas_diarias(fecha);

CREATE TABLE actas_semanales (
  id TEXT PRIMARY KEY,
  semana_obra INTEGER,
  semana_ano TEXT,
  fecha_inicio DATE,
  ...
  desviaciones_criticas JSONB,
  hitos_cumplidos JSONB,
...
**4. Despliega en Streamlit Cloud con este `secrets.toml`**
  Settings → Secrets → pega el contenido.

**5. Instala dependencia:**
```bash
